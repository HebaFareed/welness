...
class WC_Appointments_GCal {
	...
	public function sync_to_gcal( $appointment_id, $appointment_staff_id = false, $staff_calendar_id = false, $staff_sync = false ) {
		...
		if ( $padding_duration && in_array( $product->get_duration_unit(), [ 'hour', 'minute', 'day' ] ) ) {
			$appointment_data += array(
				__( 'Padding Time', 'woocommerce-appointments' ) => wc_appointment_pretty_timestamp( $padding_duration, $product->get_duration_unit() ),
			);
		}
		...
		if ( is_a( $order, 'WC_Order' ) ) {
			foreach ( $order->get_items() as $order_item_id => $order_item ) {
				...
				foreach ( $order_item->get_all_formatted_meta_data() as $meta_id => $meta ) {
					...
					$description .= apply_filters( 'woocommerce_appointments_gcal_sync_order_meta',
