...
class WC_Product_Appointment_Data_Store_CPT extends WC_Product_Data_Store_CPT {
	...
	protected function read_staff( &$product ) {
		...
		$transient_name = 'schedule_staff_ids_' . md5( http_build_query( array( $product->get_id(), WC_Cache_Helper::get_transient_version( 'appointments' ) ) ) );
		$staff_ids      = WC_Appointments_Cache::get( $transient_name );

		if ( false === $staff_ids ) {
			$staff_ids = wp_parse_id_list(
				$wpdb->get_col(
					$wpdb->prepare(
						"SELECT users.ID
			...
			// Update cache.
			WC_Appointments_Cache::set( $transient_name, $staff_ids, DAY_IN_SECONDS * 30 );
		}
		*/
