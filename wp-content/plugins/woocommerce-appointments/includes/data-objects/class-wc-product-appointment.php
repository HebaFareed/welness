...
class WC_Product_Appointment extends WC_Product {
	...
	*/
	 ...
	 */
	public function get_slots_availability( $start_date, $end_date, $qty, $staff_id, $intervals = [] ) {
		...
		foreach ( $slots as $slot ) {
			...
			if ( ! empty( $existing_appointments ) ) {
				foreach ( $existing_appointments as $existing_appointment ) {
					...
					if ( $existing_appointment->get_product_id() !== $this->get_id() && apply_filters( 'wc_apointments_check_appointment_product', true, $existing_appointment->get_id(), $this->get_id() ) ) {
					...
					$qty_to_add = apply_filters( 'wc_apointments_check_appointment_qty', $qty_to_add, $existing_appointment, $this );
			...
			if ( ! empty( $existing_appointments ) ) {
				foreach ( $existing_appointments as $existing_appointment ) {
					...
					$end_time   = $existing_appointment->get_end();
	 				...
	 				// When existing appointment is scheduled with another product,
	 				// remove all available capacity, so staff becomes unavailable for this product.
	 				if ( $appointment_product_id !== $this->get_id() && apply_filters( 'wc_apointments_check_appointment_product', true, $existing_appointment->get_id(), $this->get_id() ) ) {
						$repeat = max( 1, $original_available_qty );
	 				// Only remove capacity scheduled for existing product.
	 				} else {
	 					$repeat = max( 1, $existing_appointment->get_qty() );
	 				}
					$repeat = apply_filters( 'wc_apointments_check_appointment_qty', $repeat, $existing_appointment, $this );
	...
	*/
					...
					if ( $product_has_staff ) {
						...
						if (
							...
							foreach ( $appointment_staff_ids as $appointment_staff_id ) {
								if ( $existing_appointment->get_product_id() !== $product_id && apply_filters( 'wc_apointments_check_appointment_product', true, $existing_appointment->get_id(), $product_id ) ) {
	...
	public function get_time_slots_html( $args ) {
		...
		$timezone         = $args['timezone'];
		$include_sold_out = $args['include_sold_out'];
		$appointments     = []; #empty, included for backwards compatiblity.
