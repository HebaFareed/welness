...
class WC_Appointment_Order_Manager {
	...
	public function order_again_button( $order ) {
		if (
			! $order ||
			! $order->has_status( apply_filters( 'woocommerce_valid_order_statuses_for_order_again', array( 'completed' ) ) ) ||
			! is_user_logged_in()
		) {
			return;
		}
	...
	public function delete_post( $post_id ) {
		...
		if ( 'wc_appointment' === get_post_type( $post_id ) ) {
			...
			$order_delete = apply_filters( 'woocommerce_appointments_stop_order_delete', true, $order_id, $post_id );

			if ( 1 === $item_count && $order_id && $order_delete && ! self::is_syncing( $order_id ) ) {
				$order->delete( true );
			}

			// Fluch cache.
			WC_Appointments_Cache::clear_cron_hooks( (int) $post_id );
	...
	private function maybe_update_order( $appointment_id, $status ) {
		...
		// Stop updating order status with a filter.
		if ( apply_filters( 'woocommerce_appointments_stop_order_update', false, $order, $appointment_id, $status ) ) {
 			self::syncing_stop( $appointment_id );
 			return;
 		}
