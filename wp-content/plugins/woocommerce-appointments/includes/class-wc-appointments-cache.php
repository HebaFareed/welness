...
class WC_Appointments_Cache {
	...
	public function __construct() {
		...
		add_action( 'delete_appointment_staff_ids_transients', array( __CLASS__, 'clear_cache' ) );
	}
	...
	public static function clear_cron_hooks( $post_id = 0 ) {
		as_unschedule_action( 'wc-appointment-reminder', [ $post_id ], 'wca' );
		as_unschedule_action( 'wc-appointment-complete', [ $post_id ], 'wca' );
		as_unschedule_action( 'wc-appointment-remove-inactive-cart', [ $post_id ], 'wca' );
		as_unschedule_action( 'wc-appointment-follow-up', [ $post_id ], 'wca' );
	 ...
	 */
	public static function delete_appointment_staff_ids_transients() {
		global $wpdb;
		$limit = 500;

		$affected_timeouts   = $wpdb->query( $wpdb->prepare( "DELETE FROM {$wpdb->options} WHERE option_name LIKE %s LIMIT %d;", '_transient_timeout_schedule_staff_ids_%', $limit ) );
		$affected_transients = $wpdb->query( $wpdb->prepare( "DELETE FROM {$wpdb->options} WHERE option_name LIKE %s LIMIT %d;", '_transient_schedule_staff_ids_%', $limit ) );

		// If affected rows is equal to limit, there are more rows to delete. Delete in 10 secs.
		if ( $affected_transients === $limit ) {
			as_schedule_single_action( time() + 10, 'delete_appointment_staff_ids_transients', array( time() ), 'wca' );
		}
	}
