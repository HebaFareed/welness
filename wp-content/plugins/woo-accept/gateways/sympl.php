<?php
class WC_Gateway_Accept_Sympl extends WC_Payment_Gateway
{
    public $id;
    public $enabled;
    public $icon;
    ...
    {
        $this->id = 'accept-sympl';

        if ( !AdminHelper::valid_currency($this->id) || $this->get_option('enabled') == "no") {
            $this->enabled = false;
        }

        $this->icon               = plugins_url('/woo-accept/assets/img/Sympl.png');
        $this->has_fields         = true;
        $this->method_title       = __('Sympl');
        $this->method_description = __('Adds an option to pay with Sympl by Accept.');
        $this->method_public_description = __('Pay with your Sympl.');
        $this->order_button_text  = __('Proceed');
        $this->notify_url         = WC()->api_request_url('wc-accept-sympl');
        ...
        add_action('woocommerce_api_wc-accept-sympl', array($this, 'callback'));
    }
