<?php
class WC_Gateway_Accept_Lucky extends WC_Payment_Gateway
{
    public $id;
    public $enabled;
    public $icon;
    ...
    {
        $this->id = 'accept-lucky';

        if ( !AdminHelper::valid_currency($this->id) || $this->get_option('enabled') == "no") {
            $this->enabled = false;
        }

        $this->icon               = plugins_url('/woo-accept/assets/img/lucky.jpeg');
        $this->has_fields         = true;
        $this->method_title       = __('Lucky');
        $this->method_description = __('Adds an option to pay with Lucky by Accept.');
        $this->method_public_description = __('Pay with your Lucky.');
        $this->order_button_text  = __('Proceed');
        $this->notify_url         = WC()->api_request_url('wc-accept-lucky');
        ...
        add_action('woocommerce_api_wc-accept-lucky', array($this, 'callback'));
    }
    ...
    public function process_payment($order_id)
    {
        ...
        $url = "https://accept.paymobsolutions.com/iframe/$key";
