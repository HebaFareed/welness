<?php
class WC_Gateway_Accept_Souhoola extends WC_Payment_Gateway
{
    public $id;
    public $enabled;
    public $icon;
    ...
    {
        $this->id = 'accept-souhoola';

        if ( !AdminHelper::valid_currency($this->id) || $this->get_option('enabled') == "no") {
            $this->enabled = false;
        }

        $this->icon               = plugins_url('/woo-accept/assets/img/Souhoola.jpg');
        $this->has_fields         = true;
        $this->method_title       = __('Souhoola');
        $this->method_description = __('Adds an option to pay with Souhoola by Accept.');
        $this->method_public_description = __('Pay with your Souhoola.');
        $this->order_button_text  = __('Proceed');
        $this->notify_url         = WC()->api_request_url('wc-accept-souhoola');
        ...
        add_action('woocommerce_api_wc-accept-souhoola', array($this, 'callback'));
    }
