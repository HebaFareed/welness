<?php
class WC_Gateway_Accept_Tabby extends WC_Payment_Gateway
{
    public $id;
    public $enabled;
    public $icon;
    ...
    {
        $this->id = 'accept-tabby';

        if ( !AdminHelper::valid_currency($this->id) || $this->get_option('enabled') == "no") {
            $this->enabled = false;
        }

        $this->icon               = plugins_url('/woo-accept/assets/img/tabby.jpeg');
        $this->has_fields         = true;
        $this->method_title       = __('Tabby');
        $this->method_description = __('Adds an option to pay with Tabby by Accept.');
        $this->method_public_description = __('Pay with your Tabby.');
        $this->order_button_text  = __('Proceed');
        $this->notify_url         = WC()->api_request_url('wc-accept-tabby');
        ...
        add_action('woocommerce_api_wc-accept-tabby', array($this, 'callback'));
    }
