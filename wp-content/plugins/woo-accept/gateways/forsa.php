<?php
class WC_Gateway_Accept_Forsa extends WC_Payment_Gateway
{
    public $id;
    public $enabled;
    public $icon;
    ...
    {
        $this->id = 'accept-forsa';

        if ( !AdminHelper::valid_currency($this->id) || $this->get_option('enabled') == "no") {
            $this->enabled = false;
        }

        $this->icon               = plugins_url('/woo-accept/assets/img/forsa.jpg');
        $this->has_fields         = true;
        $this->method_title       = __('Forsa');
        $this->method_description = __('Adds an option to pay with Forsa by Accept.');
        $this->method_public_description = __('Pay with your Forsa.');
        $this->order_button_text  = __('Proceed');
        $this->notify_url         = WC()->api_request_url('wc-accept-forsa');
        ...
        add_action('woocommerce_api_wc-accept-forsa', array($this, 'callback'));
    }
