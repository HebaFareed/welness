<?php
class WC_Gateway_Accept_Premium extends WC_Payment_Gateway
{
    public $id;
    public $enabled;
    public $icon;
    ...
    {
        $this->id = 'accept-premium';

        if ( !AdminHelper::valid_currency($this->id) || $this->get_option('enabled') == "no") {
            $this->enabled = false;
        }

        $this->icon               = plugins_url('/woo-accept/assets/img/premium.png');
        $this->has_fields         = true;
        $this->method_title       = __('Premium');
        $this->method_description = __('Adds an option to pay with Premium by Accept.');
        $this->method_public_description = __('Pay with your Premium.');
        $this->order_button_text  = __('Proceed');
        $this->notify_url         = WC()->api_request_url('wc-accept-premium');
        ...
        add_action('woocommerce_api_wc-accept-premium', array($this, 'callback'));
    }
